...
describe("Smoke-Property: Semantics role coverage (short run)", () => {
  it("quickly generates frames and relations and checks validators", async () => {
    const rng = makeRng(424242)
    const iterations = 6
    for (let i = 0; i < iterations; i++) {
      const { service, db, dispose } = await createSemanticsTestHarness()
      try {
        const roleCount = Math.floor(rng() * 3)
        const roles = [] as any[]
        ...
        const frame = await service.createFrame({
          ...
          slug: `f${i}`,
          domain: "test",
          description: "short property test",
          roles
        })
